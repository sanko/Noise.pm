...
C<Protocol::Noise> implements the Noise Protocol Framework, a modern cryptographic construction for building
...
=head2 SymmetricState

Manages the chaining key (C<ck>) and the handshake hash (C<h>). It ensures that every message sent or received
...
=item C<prologue>: Optional initial data to be hashed into the state.

=back

=head2 C<initialize_state( $protocol_name )>

Sets up the state machine for a specific protocol. Defaults to C<Noise_XX_25519_ChaChaPoly_SHA256>.

=head2 C<mix_hash( $data )>

Incorporates C<$data> into the handshake hash.
...
=head2 C<split_for_transport( )>

Finalizes the handshake and returns two 32-byte keys for secure communication.

=head1 Handshake Patterns

While this module provides low-level primitives, it is recommended to use higher-level wrappers (like
L<Net::Libp2p::Protocol::Noise>) for specific application integrations.

=head1 SEE ALSO

L<Protocol::Noise::Stream>, L<https://noiseprotocol.org/>

L<Crypt::Protocol::Noise> is an earlier implementation.
