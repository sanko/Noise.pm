...
class Protocol::Noise::SymmetricState v0.0.1 {
    ...
    field $h            : reader;
    field $ck           : reader;
    field $cipher_state : reader;
    field $hash         : param = 'SHA256';
    field $cipher       : param;
    ADJUST {
        die 'Cipher not specified' unless $cipher;
        $cipher_state = Protocol::Noise::CipherState->new( cipher => $cipher );
    }
    ...
    method split () {
        ...
        my $c1 = Protocol::Noise::CipherState->new( cipher => $cipher );
        $c1->set_key( substr( $out1, 0, 32 ) );
        my $c2 = Protocol::Noise::CipherState->new( cipher => $cipher );
